                    ...
                    if(count($hideprice) > 0) {
                        foreach ($hideprice as $key => $_hideprice) {
                            if($_hideprice['entity_id'] == $_product->getId()) {
                                ...
                                if((in_array($customer_group_current,$customer_group) == true || ((in_array(0,$customer_group) ==true && $isLoggedIn ==false))) && (in_array($this->helper->getStoreId(), $store_id) == true || in_array(0,$store_id ) == true)) {
